...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene, kuavo_robot: KuavoRobotController):
    ...
    while simulation_app.is_running():
        ...
        if not FIRST_TIME_FLAG:
            ...
            timing_stats["sensor_data"] += time.time() - sensor_start_time
            ...
            if current_cmd is not None:
                ...
                for i in range((len(kuavo_robot._leg_idx)//2)): # type: ignore
                    full_torque_cmd[kuavo_robot._leg_idx[2*i]] = current_cmd.tau[i]  # type: ignore # 左腿
                    full_torque_cmd[kuavo_robot._leg_idx[2*i+1]] = current_cmd.tau[i+6]  # type: ignore # 右腿

                # 手部                    
                for i in range((len(kuavo_robot._arm_idx)//2)): # type: ignore
                    full_torque_cmd[kuavo_robot._arm_idx[2*i]] = current_cmd.tau[12+i]  # type: ignore # 左臂
                    full_torque_cmd[kuavo_robot._arm_idx[2*i+1]] = current_cmd.tau[19+i]  # type: ignore # 右臂
                
                # 头部
                full_torque_cmd[kuavo_robot._head_idx[0]] = current_cmd.tau[26]  # type: ignore # head_l1_joint
                full_torque_cmd[kuavo_robot._head_idx[1]] = current_cmd.tau[27]  # type: ignore # head_r1_joint
...
def main():
    ...
    # # 关闭高速ccd碰撞检测（检测高速球体/弹丸）
    # sim._physics_context.enable_ccd(False) 
    # # 关闭场景查询支持
    # sim._physics_context.set_enable_scene_query_support(False)
    # # 关闭求解器的额外稳定化支持
    # sim._physics_context.enable_stablization(False)
